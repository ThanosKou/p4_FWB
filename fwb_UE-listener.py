...
def handle_pkt(pkt):
    ...
    if fwb in pkt:
        if pkt[IP].dst == '10.0.2.2' and pkt[TCP].dport == 1111: # 1111 data layer
            ...
            if (pkt[TCP].sport == 51003 or pkt[fwb].dst_id in a_m_idx[prev_dst]) and pkt[fwb].pkt_id not in received_packets:
                received_packets.append(pkt[fwb].pkt_id)
                last_received = pkt[fwb].pkt_id
                print('{},{},{},{},{}\n'.format(last_received,generated_time,time.time()-t0-generated_time,pkt[fwb].dst_id,pkt[IP].src))
            ...
            if last_received == event_idx:
                ...
                if next_dst == 4:
                    ...
                    prev_dst = 4
                    next_dst = int(np.random.choice(np.array(transitions[prev_dst])))
                    print('outage and next link state is:', next_dst)
...
def main():
    ...
    record_string = Path.cwd()/"out_data"/"fwb_pkt_arrivals_{}ms_{}ms.txt".format(GW_delay,UE_delay)
