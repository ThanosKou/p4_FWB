...
def buffer_pkt_extract(my_buffer,my_buffer_gen_times,ue_asks):
    global iface
    global current_dst_id
    to_send = [_ for _ in my_buffer if _ >= ue_asks]
    for idx in to_send:
	gen_time = str(my_buffer_gen_times[idx])
        buff_pkt = e / fwb(dst_id=current_dst_id, pkt_id=idx, pid=TYPE_IPV4)/ pkt_barebone / gen_time
...
def handle_pkt(pkt):
    ...
    if fwb in pkt:
        if pkt[IP].src == '10.0.2.2' and pkt[TCP].dport==2222: #control packet
            ...
            buffer_pkt_extract(my_buffer,my_buffer_gen_times,ue_asks)
            ...
            if current_state == 'secondary':
                ...
                my_buffer_gen_times[w_idx] = bytes(pkt[TCP].payload)    
...
def main():
    ...
    my_buffer_gen_times = [0]*BUFFER_LEN
    w_idx = 0
    prev_state = 'primary'
    current_state = 'primary'
    prev_destination = -1
