...
def compile_data(data_path):
    data_path = '/home/thanos/tutorials/exercises/p4_FWB/out_data/realistic_traffic_model/exponential'
    files = os.listdir(data_path)
    gpp3_files = [file for file in files if file.startswith('3gpp_') and file.endswith('_50ms.txt')]
    fwb_files = [file for file in files if file.startswith('pkt') and file.endswith('_50ms.txt')]
    ...
    for gpp_file in gpp3_files:
	    ...
	    flow_generating_time = float(readings[1])
            flow_time = float(readings[2])
	    flow_delay = flow_time -t0 - flow_generating_time
            flow_dst_id = float(readings[3])
            write_str = '{},{},{},{},{},{},{},{}\n'.format(flow_idx, flow_time - t0, flow_generating_time, flow_delay, flow_dst_id, gw_delay, ue_delay,
                                                     protocol_name)
            wf.write(write_str)
        rf.close()
    protocol_name = 'FWB'
